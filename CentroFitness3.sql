...
--
-- Host: localhost
-- Creato il: Giu 26, 2021 alle 17:49
...
(20, 'CCCAAA123456DDDB', 'Carlo', 'Alberti', '2016-08-16', 'Via Alberti', 'Catania', 'cccaaa@live.it', '3467890123', '$2y$10$df45xJas5oU33Vh4qJxmhuMM11iD2g3nHfdkL64UtnsL5b5othpQO');
...
(12, '2021-05-01', NULL, 1200, 12, 2),
(13, '2021-04-06', NULL, 1400, 13, 2);
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `centrofitness3`.`impiegoimpiegato`  AS SELECT `I`.`ID_impiegato` AS `ID_impiegato`, `IM`.`Nome` AS `Nome`, `IM`.`Cognome` AS `Cognome`, `IM`.`C_F` AS `C_F`, `I`.`Sede` AS `Sede` FROM (`centrofitness3`.`impiego` `I` join `centrofitness3`.`impiegato` `IM` on(`I`.`ID_impiegato` = `IM`.`ID_impiegato`)) ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `centrofitness3`.`iscrittocorso`  AS SELECT `I`.`ID_iscritto` AS `ID_iscritto`, `I`.`Nome` AS `Nome`, `I`.`Cognome` AS `Cognome`, `I`.`Data_nascita` AS `Data_nascita`, `I`.`età` AS `età`, `I`.`C_F` AS `C_F`, `I`.`Indirizzo` AS `Indirizzo`, `I`.`Citta` AS `Citta`, `I`.`Telefono` AS `Telefono`, `I`.`Posta_elettronica` AS `Posta_elettronica`, `C`.`ID_corso` AS `ID_corso`, `C`.`Nome_corso` AS `Nome_corso`, `C`.`Prezzo` AS `Prezzo`, `C`.`Max_iscritti` AS `Max_iscritti`, `C`.`Fascia_eta` AS `Fascia_eta` FROM ((`centrofitness3`.`corso` `C` join `centrofitness3`.`abbonamento` `A` on(`C`.`ID_corso` = `A`.`Corso`)) join `centrofitness3`.`iscritto` `I` on(`I`.`ID_iscritto` = `A`.`Iscritto`)) ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `centrofitness3`.`n_usi`  AS SELECT `U`.`Attrezzo` AS `Attrezzo`, `i`.`Citta` AS `Citta`, count(`i`.`ID_iscritto`) AS `usi` FROM (`centrofitness3`.`iscrittocorso` `I` join `centrofitness3`.`usufruire` `U` on(`i`.`ID_corso` = `U`.`Corso`)) GROUP BY `i`.`Citta`, `U`.`Attrezzo` ;
...
--
ALTER TABLE `CORSO`
  MODIFY `ID_corso` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=33;

--
-- AUTO_INCREMENT per la tabella `IMPIEGATO`
--
ALTER TABLE `IMPIEGATO`
  MODIFY `ID_impiegato` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=21;

--
-- AUTO_INCREMENT per la tabella `IMPIEGO`
--
ALTER TABLE `IMPIEGO`
  MODIFY `ID_impiego` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=14;
